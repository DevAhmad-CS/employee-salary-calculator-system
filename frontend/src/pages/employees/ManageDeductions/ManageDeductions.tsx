/**
 * Manage Deductions Page
 * 
 * Allows managing deductions for a specific employee.
 * Includes a table of current deductions and a form to add new ones.
 */
...
import styles from './ManageDeductions.module.css';
...
export default function ManageDeductions() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [employeeResponse, deductionsResponse] = await Promise.all([
          employeeService.getById(parseInt(id)),
          deductionsService.getByEmployeeId(parseInt(id)),
        ]);
  ...
  const onSubmit = async (data: DeductionFormData) => {
    ...
    try {
      setError('');
      const response = await deductionsService.create({
      ...
      setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error || (err instanceof Error ? err.message : 'Unknown error') || 'Failed to create deduction');
    }
  };
  ...
  const handleUpdate = async (deductionId: number) => {
    if (!editFormData) return;

    try {
      setError('');
      const response = await deductionsService.update(deductionId, {
        type: editFormData.type,
        amount: editFormData.amount,
      });
      setDeductions(deductions.map((d) => (d.id === deductionId ? response.data : d)));
      setEditingId(null);
      setEditFormData(null);
    } catch (err: unknown) {
      setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error || (err instanceof Error ? err.message : 'Unknown error') || 'Failed to update deduction');
    }
  };
  ...
  const handleDelete = async (deductionId: number) => {
    ...
    try {
      setError('');
      await deductionsService.delete(deductionId);
      setDeductions(deductions.filter((d) => d.id !== deductionId));
    } catch (err: unknown) {
      setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error || (err instanceof Error ? err.message : 'Unknown error') || 'Failed to delete deduction');
    }
  };
  ...
  if (loading) {
    return (
      <div className={styles.manageDeductionsContainer}>
        <Sidebar />
        ...
        <main className={styles.mainContent}>
          ...
          <div className={styles.listSection}>
                  ...
                  {deductions.length === 0 ? (
                    <tr>
                      <td colSpan={3} className={styles.emptyMessage}>
                        No deductions found. Add one below.
                      </td>
                    </tr>
                  ) : (
                      ...
                      <tr key={deduction.id}>
                        <td>
                          {editingId === deduction.id ? (
                            ...
                            <span className={styles.badgeDeduction}>{deduction.type}</span>
                          )}
                        </td>
            ...
            <div className={styles.totalSection}>
              ...
              <strong>Total Deductions: <span className={styles.totalAmount}>{formatCurrency(totalDeductions)}</span></strong>
            </div>
          </div>
