/**
 * Manage Allowances Page
 * 
 * Allows managing allowances for a specific employee.
 * Includes a table of current allowances and a form to add new ones.
 */
...
import styles from './ManageAllowances.module.css';
...
export default function ManageAllowances() {
  ...
  const [editFormData, setEditFormData] = useState<{ type: string; amount: number } | null>(null);
  ...
  // Fetch employee and allowances data
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [employeeResponse, allowancesResponse] = await Promise.all([
          employeeService.getById(parseInt(id)),
          allowancesService.getByEmployeeId(parseInt(id)),
        ]);
  ...
  const onSubmit = async (data: AllowanceFormData) => {
    ...
    try {
      setError('');
      const response = await allowancesService.create({
      ...
      const axiosError = err as { response?: { data?: { error?: string } } };
      const errorMessage = err instanceof Error ? err.message : 'Failed to create allowance';
      setError(axiosError?.response?.data?.error || errorMessage);
    }
  };
  ...
  const handleUpdate = async (allowanceId: number) => {
    if (!editFormData) return;

    try {
      setError('');
      const response = await allowancesService.update(allowanceId, editFormData);
      setAllowances(allowances.map((a) => (a.id === allowanceId ? response.data : a)));
      setEditingId(null);
  ...
  const handleDelete = async (allowanceId: number) => {
    ...
    try {
      setError('');
      await allowancesService.delete(allowanceId);
      setAllowances(allowances.filter((a) => a.id !== allowanceId));
    } catch (err: unknown) {
      const axiosError = err as { response?: { data?: { error?: string } } };
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete allowance';
      setError(axiosError?.response?.data?.error || errorMessage);
    }
  };
  ...
  if (loading) {
    return (
      <div className={styles.manageAllowancesContainer}>
        <Sidebar />
        ...
        <main className={styles.mainContent}>
          ...
          <div className={styles.listSection}>
                  ...
                  {allowances.length === 0 ? (
                    <tr>
                      <td colSpan={4} className={styles.emptyMessage}>
                        No allowances found. Add one below.
                      </td>
                    </tr>
                  ) : (
                      ...
                      <tr key={allowance.id}>
                        <td>
                          {editingId === allowance.id ? (
                            ...
                            <span className={styles.badgeAllowance}>{allowance.type}</span>
                          )}
                        </td>
                        ...
                        <td>
                          {editingId === allowance.id ? (
                            ...
                            <span style={{ color: 'var(--text-light)', fontStyle: 'italic' }}>No notes</span>
                          )}
                        </td>
                        <td>
                            ...
                            {editingId === allowance.id ? (
                              <>
                                <button
                                  ...
                                  onClick={() => handleUpdate(allowance.id)}
                                >
            ...
            <div className={styles.totalSection}>
              ...
              <strong>Total Allowances: <span className={styles.totalAmount}>{formatCurrency(totalAllowances)}</span></strong>
            </div>
          </div>
