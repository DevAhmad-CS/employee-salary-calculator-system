/**
 * Salary Slip Preview Page
 * 
 * Displays a preview of a salary slip before saving.
 * Matches mockup design exactly with Save, Print, Download PDF buttons.
 */
...
import styles from './SlipPreview.module.css';
...
export default function SlipPreview() {
  ...
  // Get slip data from location state (if coming from GenerateSlip) or fetch by ID
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if slip data is passed from GenerateSlip page
        const locationState = location.state as { slip?: SalarySlip; employee?: Employee } | null;
        
        if (locationState?.slip) {
          // Use slip data from location state
          const slipData = locationState.slip;
          setSlip(slipData);
          
          if (locationState.employee) {
            setEmployee(locationState.employee);
          } else {
            const employeeResponse = await employeeService.getById(slipData.employeeId);
            setEmployee(employeeResponse.data);
          }

          // Fetch allowances, deductions, and bonuses
          try {
          ...
          setError('No salary slip data available');
        }
      } catch (err: unknown) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If slip doesn't have an ID, it means it's a preview from GenerateSlip
      // In this case, we need to generate it first
      if (!slip.id) {
        ...
        if (response.success) {
          // Navigate to the saved slip details
          navigate(`/salary-slips/${response.data.id}`);
        } else {
          setError('Failed to save salary slip');
        }
      } else {
        // Slip already exists, just navigate to it
        navigate(`/salary-slips/${slip.id}`);
      }
    } catch (err: unknown) {
      setError((err as { response?: { data?: { error?: string } } })?.response?.data?.error || (err instanceof Error ? err.message : 'Unknown error') || 'Failed to save salary slip');
    } finally {
      setSaving(false);
    }
  };
  ...
  // Handle download PDF (placeholder - would need PDF generation library)
  const handleDownloadPDF = () => {
    // TODO: Implement PDF download functionality
    alert('PDF download functionality will be implemented in Phase 3');
  };
  ...
  if (loading) {
    return (
      <div className={styles.slipPreviewContainer}>
        <Sidebar />
        <div className={styles.mainContentWrapper}>
          <Header />
          <main className={styles.mainContent}>
            <div className={styles.loadingMessage}>
              <p>Loading salary slip preview...</p>
            </div>
          </main>
        </div>
      </div>
    ...
    <div className={styles.slipPreviewContainer}>
      ...
      <div className={styles.mainContentWrapper}>
        ...
        <main className={styles.mainContent}>
          ...
          <div className={styles.pageHeader}>
            <div>
              ...
              <p className={styles.pageSubtitle}>
                <svg className={styles.previewIcon} width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M1 12S5 4 12 4S23 12 23 12S19 20 12 20S1 12 1 12Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  <path d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                Review the salary slip before saving
              </p>
            </div>
          ...
          {/* Salary Slip Preview Card - Same structure as SlipDetails */}
