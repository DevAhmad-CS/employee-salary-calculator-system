 ...
 * Monthly Report Page
 * 
 * Displays monthly financial report with statistics and employee breakdown.
...
export default function MonthlyReport() {
  ...
  const [employeeBreakdown, setEmployeeBreakdown] = useState<SalarySlipWithEmployee[]>([]);
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        setEmployeeBreakdown(slipsWithEmployees);
      } catch (err: unknown) {
  ...
  // Filter employee breakdown
  const filteredBreakdown = employeeBreakdown.filter((item) => {
    if (quickDepartment && item.employee?.department !== quickDepartment) {
      return false;
    }
    if (quickEmployeeSearch.trim()) {
      ...
      return (
        ...
        formatEmployeeId(item.employeeId).toLowerCase().includes(query)
      );
    }
    return true;
  });
  ...
  const totals = filteredBreakdown.reduce(
    (acc, item) => {
      acc.basicSalary += item.basicSalary;
  ...
  const handleApplyQuickFilters = () => {
    // Filters are applied via filteredBreakdown
  };
  ...
  // Get active filters from report
  const activeFilters: string[] = [];
  if (report?.statistics.departmentBreakdown && report.statistics.departmentBreakdown.length > 0) {
    // If there's only one department, show it as active filter
    if (report.statistics.departmentBreakdown.length === 1) {
      activeFilters.push(`Department: ${report.statistics.departmentBreakdown[0].department}`);
    }
  }

  if (loading) {
    return (
      <div className={styles.monthlyReportContainer}>
        <Sidebar />
        ...
        <main className={styles.mainContent}>
          ...
          <div className={styles.summaryCards}>
            <div className={styles.summaryCard}>
              ...
              <p className={styles.summaryLabel}>Active employees in {report.period.monthName}</p>
            </div>
            <div className={styles.summaryCard}>
              <div className={styles.summaryCardHeader}>
                <svg className={styles.summaryIcon} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className={styles.summaryLabel}>Total monthly payroll</p>
            </div>
            <div className={styles.summaryCard}>
              <div className={styles.summaryCardHeader}>
                <svg className={styles.summaryIcon} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className={styles.summaryLabel}>Per employee average</p>
            </div>
          </div>
          ...
          <div className={styles.employeeTableSection}>
            <div className={styles.tableSection}>
              <div className={styles.sectionHeader}>
                ...
                <h3>Employee Salary Breakdown</h3>
              </div>
                ...
                <table className={styles.reportTable}>
                  ...
                  <tbody>
                    {filteredBreakdown.map((item) => (
                      <tr key={item.id}>
                        <td className={styles.employeeId}>{formatEmployeeId(item.employeeId)}</td>
                        <td>{item.employee?.fullName || 'Unknown'}</td>
                        <td>
                          {item.employee?.department && (
                            <span className={`${styles.badge} ${getDepartmentBadgeClass(item.employee.department)}`}>
                              {item.employee.department}
                            </span>
                          )}
                        </td>
                        <td>{item.employee?.position || 'N/A'}</td>
                        <td className={styles.salaryAmount}>{formatCurrency(item.basicSalary)}</td>
                        <td className={`${styles.salaryAmount} ${styles.positive}`}>
                          {formatCurrency(item.totalAllowances)}
                        </td>
                        <td className={`${styles.salaryAmount} ${item.totalBonus > 0 ? styles.bonus : ''}`}>
                          {formatCurrency(item.totalBonus)}
                        </td>
                        <td className={`${styles.salaryAmount} ${styles.negative}`}>
                  ...
                  <tfoot>
                    <tr className={styles.tableTotal}>
                      ...
                      <td className={styles.salaryAmount}><strong>{formatCurrency(totals.basicSalary)}</strong></td>
                      <td className={`${styles.salaryAmount} ${styles.positive}`}>
                        <strong>{formatCurrency(totals.totalAllowances)}</strong>
                      </td>
                      <td className={`${styles.salaryAmount} ${styles.bonus}`}>
                        <strong>{formatCurrency(totals.totalBonus)}</strong>
                      </td>
                      <td className={`${styles.salaryAmount} ${styles.negative}`}>
